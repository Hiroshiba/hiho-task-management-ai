...
function resetProposalState(): void {
  ...
  awaitingEditResult.value = false;
  localError.value = "";
}
...
watch(
  () => props.editResult,
  (result) => {
    if (!awaitingEditResult.value || result == null) {
      return;
    }
    if (result.proposal_id !== props.proposal.proposal_id) {
      return;
    }
    awaitingEditResult.value = false;
...
function statusEvidenceDetails(operation: ProposalOperation): readonly string[] {
