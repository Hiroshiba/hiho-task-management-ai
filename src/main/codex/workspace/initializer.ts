...
- 変更案を返す場合は、プロンプトで指定された提案ファイルを読み取り専用の文脈として参照し、response_jsonへproposal_file_idと完全な変更案JSONを返してください。
- 提案ファイルのパスとproposal_file_idは今回のターン専用です。ファイルへ変更案や会話内容を書き込まないでください。
- 提案ファイルを変更、削除、移動、置換しないでください。完全な新しい変更案は提案ファイルへ書き込まず、response_jsonのproposalへ返してください。
...
- \`<proposal_file>\` が提示された場合は、提案ファイルの既存操作を読み取り、利用者が明示的に変更、取消し、または対象訂正をした操作を除き、その全操作を引き継いで改訂してください。承認前の操作を更新済みと説明しないでください。
