...
- 変更案を返す場合は、プロンプトで指定された提案ファイルを読み、完全な変更案JSONへ更新してください。最終応答には変更案JSONを埋め込まず、指定されたproposal_file_idだけを返してください。
- 提案ファイルのパスとproposal_file_idは今回のターン専用です。指定されたファイル以外へ変更案や会話内容を書き込まないでください。
