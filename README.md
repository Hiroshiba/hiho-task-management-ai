...
自動更新に対応しない版から移るときや、自動更新が失敗したときは、次の手順で手動導入します。`0.1.1`など起動できる版では、更新前の版と設定を[版と設定の確認](#版と設定の確認)に沿って記録してください。`0.1.2`が起動できない場合は、更新前の確認を省いて導入を進め、導入後に版と設定を確認してください。
...
初回導入後と更新後に、実際に起動するアプリの版表記を記録します。起動できる版では更新前も記録してください。macOSでは`/Applications/TaskHub.app/Contents/Info.plist`の`CFBundleShortVersionString`、Windowsではインストール先の`TaskHub.exe`のプロパティの「詳細」にあるファイルバージョンを確認します。prereleaseを含む版表記は実成果物で未確認です。配布版と実際の成果物の版表記の対応を確かめてから照合し、表記から次版を識別できない場合は版の確認を未完了として記録してください。

起動できる版の設定保持の検証では、初回導入後にObsidianのVaultを登録し、ヘッダーの「設定」に表示されるVault IDと絶対パスを更新前後で比較します。更新後に初回設定をやり直さず起動でき、登録値が保持されることを確認してください。Asanaのタスクが表示されることだけでは、ローカルデータの保持を確認したことにはなりません。設定・データの保存先は配布形式によって変えませんが、自動更新時の保持は2版間の実機で未検証です。

`0.1.2`が起動できない場合は、`0.1.3`の導入後に上記の版表記と「設定」のVault ID・絶対パスを記録し、初回設定をやり直さず既存の設定・データを利用できることを確認してください。更新前に記録できなかった値との比較は未実施として記録してください。
